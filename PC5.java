            ...
            if (table.getPhaseJouer()) // cas on est sur la phase de jeu
            {
                if (joueurCourant == this.id) // le joueurCourant execute le minmax
                {
                    ...
                    if (getCarteAdv() == null)// le cas ou le joueur courant dois poser en premier une carte
                    {
                        for (Carte c : getMain().getMain()) {
                            ...
                            } else { //cas joueur courant est l'adversaire
                                tSim.setCarte2(c);
                        ...
                        return alpha;
                    } else { // cas ou une carte à deja etait pose par l'adversair
                        for (Carte c : getMain().getMain()) {
                            if (getId() == 1) {//cas joueur courant est le joueur1
                                tSim.setCarte1(c);
                                tSim.getMain1().supp(c);
                            ...
                            } else {//cas joueur courant est le joueur2
                                tSim.setCarte2(c);
                                tSim.getMain2().supp(c);
                                tSim.getMain2connue().supp(c);
                                tSim.setPhaseJouer(false);
                    ...
                    //------------------------------------------------
                } else { // le joueurCourant veut faire perdre le joueur qui exe minmax (le Min)
                    int beta = 100;
                    Table tSim = new Table();
                    tSim = t.clone();
                    if (getCarteAdv() == null)// le cas ou le joueur courant dois poser en premier une carte
                ...
                if (!t.pilesVides()) {
                    if (joueurCourant == this.id) // le joueurCourant execute le minmax
                    {
                        ...
                        if (getId() == 1) {
                            if (tSim.getInfoAdv2().getaChoisi()) {//le joueur 2 n'a pas encore choisi
                                for (int i = 0; i < 6; i++) {
