...
public class ZoneDessin extends JComponent {
    ...
    public void paint(Graphics g) {
        ...
        //--- Dessin des cartes du joueur local ---//
        
        
        Main mainLocale = t.main1;
        
        /*int numMain = 1;
        
        if(jeu.getMode() == 2 && !jeu.getIsCroupier()) {
            numMain = 2;
            mainLocale = t.main2;
        }*/
        
        for (int f = 0; f < mainLocale.getSize(); f++) {
        
            int mid = (int) ((width / 2) - (((mainLocale.getSize() + 1) * (cw) / 2) * 0.5)) + (f * cw) / 2;
	        int up = height - ch -bh;
            Carte c = mainLocale.getCarte(f);
